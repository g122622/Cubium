...
\caption{Trident Vulkan渲染管线架构。游戏线程提交区块更新到MeshBuildScheduler，调度器按优先级分发任务到MeshWorkerPool。工作线程构建网格后，通过上传队列异步传输到GPU。Vulkan帧渲染流程使用信号量和栅栏实现CPU-GPU同步。}
...
\textbf{工作线程池}：\texttt{MeshWorkerPool}管理多个工作线程，每个线程独立构建网格。线程数根据硬件自动配置，为$(n-1)/2$，预留资源给渲染主线程。
