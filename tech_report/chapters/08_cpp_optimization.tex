...
\textbf{优化思路：}将高度图、biome 和 section 数据改为直接写入目标缓冲区，配合\texttt{PacketDeserializer::readBytesInto} 与\texttt{ChunkSection::deserialize} 消除中间 vector；同时让\texttt{ChunkSection::serialize} / 反序列化使用连续内存和一次性\texttt{memcpy} 处理 blockStates 与光照数据。这样做虽然不是典型的 SIMD 向量化，但能显著减少分配次数、缓存失效和分支开销，更适合这类内存密集型工作负载。
...
\textbf{结果解读}：从追踪结果看，这条路径更像内存带宽受限而非算术受限。优化后，\texttt{deserializeChunk} 的外层墙钟时间明显下降，而自身指令复杂度没有明显增加，说明改动是通过更好的数据布局和更少的拷贝拿到的收益，而不是单纯依赖编译器自动向量化。
