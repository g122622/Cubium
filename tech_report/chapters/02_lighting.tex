...
\begin{enumerate}
    ...
    \item \textbf{状态管理复杂}：存在多个中间状态（如\texttt{INIT}、\texttt{QUEUED}、\texttt{IN\_PROGRESS}等），状态转换逻辑复杂且容易出错
...
光照传播使用两个FIFO数组队列（增加队列和减少队列），每个队列元素编码为64位整数，如图\ref{fig:lighting_encoding}所示。

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{images/lighting_system.png}
\caption{光照系统核心设计。上图为64位队列元素编码格式，将坐标偏移、光照等级、传播方向和标志位紧凑编码。下图为双队列传播算法示意图，增亮队列和减暗队列协同工作，实现$O(n)$复杂度的光照传播。}
\label{fig:lighting_encoding}
\end{figure}
