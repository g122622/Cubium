...
### 实体系统 (`src/common/entity/`)

**核心实体类型**
- `Entity` - 实体基类，位置/速度/碰撞/乘客系统/传送门/生命周期
- `LivingEntity` - 生物实体，生命值/属性/装备/药水效果/战斗追踪
- `MobEntity` - AI 生物实体，目标选择器/控制器/寻路
- `CreatureEntity` - 可移动生物
- `AgeableEntity` - 可成长实体（年龄/繁殖）
- `Player` - 玩家实体，游戏模式/饥饿值/经验/背包/能力

**实体组件**
- `EntityDataManager` - 实体数据管理器，客户端-服务端数据同步
- `DataParameter<T>` - 类型安全的数据参数键
- `AttributeMap` - 属性映射表
- `AttributeInstance` - 属性实例（基础值 + 修改器）
- `Attributes` - 标准属性（maxHealth, followRange, movementSpeed, attackDamage 等）

**AI 系统**
- `Goal` - AI 目标基类
- `GoalSelector` - 目标选择器
- `Brain<E>` - 高级 AI 系统（记忆/传感器/日程/活动）
- `MovementController` - 移动控制器
- `PathNavigator` - 路径导航器

**实体管理**
- `EntityManager` - 实体管理器（创建/销毁/查询/遍历）
- `ClientEntityManager` - 客户端实体管理器（本地玩家/插值/动画）
- `EntityRegistry` - 实体类型注册表
- `EntityType` - 实体类型定义（工厂模式/属性）

### 命令系统 (`src/common/command/`)

**命令框架**
- `CommandDispatcher<S>` - 命令解析和执行引擎
- `CommandNode<S>` - 命令节点（Root/Literal/Argument）
- `CommandContext<S>` - 解析后的命令状态
- `CommandSource` - 命令执行上下文（执行者/位置/世界/权限）
- `StringReader` - 基于游标的字符串解析器

**参数类型** (`src/common/command/arguments/`)
- `StringArgumentType` - 字符串参数（SingleWord/QuotablePhrase/GreedyPhrase）
- `IntegerArgumentType`, `FloatArgumentType`, `BoolArgumentType` - 数值/布尔参数
- `EnumArgumentType<T>` - 枚举参数模板
- `EntityArgumentType` - 实体选择器（@p, @a, @e, @r, @s, 玩家名）
- `GameModeArgumentType`, `BlockPosArgumentType`, `Vec3ArgumentType` - 游戏参数
- `ItemArgumentType`, `ItemPredicateArgumentType` - 物品参数

**命令注册** (`src/server/command/`)
- `CommandRegistry` - 命令注册中心（单例）
- `ServerCommandSource` - 服务端命令源实现

**已实现命令**: `/gamemode`, `/tp`, `/teleport`, `/give`, `/time`, `/kill`, `/clear`, `/seed`, `/list`, `/help`, `/kick`, `/difficulty`, `/defaultgamemode`, `/weather`, `/xp`, `/experience`, `/say`, `/stop`, `/setidletimeout`

### 容器和菜单系统 (`src/common/entity/inventory/`)

**容器基类**
- `IInventory` - 背包接口
- `Container` - 客户端-服务端容器同步基类
- `AbstractContainerMenu` - GUI 菜单基类
- `Slot` - 槽位类（包含索引/背包引用/放置检查）
- `ArmorSlot`, `ResultSlot` - 特殊槽位

**玩家背包**
- `PlayerInventory` - 玩家背包（41 槽位：快捷栏 9 + 主背包 27 + 盔甲 4 + 副手 1）
- `CraftingInventory` - 合成网格背包（2x2 或 3x3）
- `CraftResultInventory` - 合成结果槽位

**具体容器**
- `ChestContainer` - 箱子容器（单箱 27 槽位 / 双箱 54 槽位）
- `FurnaceContainer` - 熔炉容器（输入/燃料/输出）
- `HopperContainer` - 漏斗容器（5 槽位）
- `CraftingMenu` - 3x3 工作台菜单
- `InventoryCraftingMenu` - 2x2 玩家背包合成菜单

**容器同步**
- `ContainerPacketHandler` - 容器数据包处理器
- `InventoryManager` - 服务端玩家背包管理
- `ContainerManager` - 服务端容器交互管理

### 光照系统 (`src/common/world/lighting/`)

**光照引擎**
- `StarLightEngine` - 光照引擎基类（Starlight 优化实现）
- `BlockStarLightEngine` - 方块光引擎（光源传播）
- `SkyStarLightEngine` - 天空光引擎（无衰减向下传播）
- `WorldLightManager` - 光照管理器（协调方块光/天空光）

**光照存储**
- `NibbleArray` - 简单 4 位存储（0-15 范围）
- `SWMRNibbleArray` - 单写多读 nibble 数组（线程安全）
- `EmptinessMap` - 区块段空隙追踪

**光照工具**
- `LightEngineUtils` - 引擎工具（位置编码/方向位操作）
- `InternalLightUtils` - 游戏机制光照计算（天空变暗/亮度计算）
- `LightType` - 光照类型枚举（SKY, BLOCK）
- `LightConstants` - 光照常量（MAX_LIGHT=15, MIN_LIGHT=0）

### UI 系统 (`src/client/ui/`)

**Kagero UI 框架** (`src/client/ui/kagero/`)
- `Widget` - UI 组件基类（生命周期/事件处理/布局属性）
- `IWidgetContainer` - 容器接口
- `ContainerWidget` - 通用容器组件
- `EventBus` - 事件总线（订阅/发布/过滤）
- `FlexLayout` - Flex 布局算法（Direction/JustifyContent/Align/Wrap）
- `LayoutEngine` - 布局引擎

**内置 Widget**
- `ButtonWidget`, `ImageButtonWidget` - 按钮
- `TextWidget` - 文本显示
- `CheckboxWidget` - 复选框
- `ListWidget` - 列表
- `ScrollableWidget` - 可滚动容器
- `SliderWidget` - 滑块
- `TextFieldWidget` - 文本输入框

**Minecraft UI** (`src/client/ui/minecraft/`)
- `Screen` - 屏幕基类
- `AbstractContainerScreen<Menu>` - 容器屏幕模板
- `ContainerScreen`, `InventoryScreen`, `MainMenuScreen`, `PauseScreen`, `OptionsScreen` - 具体屏幕
- `CraftingScreen`, `ChestScreen`, `FurnaceScreen`, `CreativeScreen` - 游戏屏幕
- `DebugScreenWidget` - F3 调试屏幕
- `TargetInfoWidget` - 目标信息显示
- `HudWidget`, `ChatWidget`, `CrosshairWidget`, `HotbarWidget` - HUD 组件

**字体系统** (`src/client/ui/`)
- `Font` - 字体类，支持多字形提供者
- `FontRenderer` - 字体渲染器（阴影/粗体/斜体/颜色）
- `IGlyphProvider` - 字形提供者接口
- `BitmapGlyphProvider` - 位图字形提供者
