 ...
 */
class PistonBlockTestWorld final : public IWorld {
public:
    PistonBlockTestWorld() : m_random(12345) {
     ...
     * @param size 边界直径
     */
    void setWorldBorderSize(double size) {
        m_worldBorder.setSize(size);
    }
...
TEST_F(PistonBlockTest, CanPush_BlockInsideWorldBorder_ReturnsTrue) {
    ...
    // 空气可以推动
    EXPECT_TRUE(PistonBlock::canPush(
        stoneState, m_world, pos,
        Direction::Up, true, Direction::Up));
}
...
TEST_F(PistonBlockTest, CanPush_HeightLimit) {
    ...
    EXPECT_TRUE(PistonBlock::canPush(
        stoneState, m_world, posInHeight,
        Direction::Up, true, Direction::Up));

    // 超出高度范围的位置（负高度）
    BlockPos posBelowHeight(0, -1, 0);
    EXPECT_FALSE(PistonBlock::canPush(
        stoneState, m_world, posBelowHeight,
        Direction::Up, true, Direction::Up));
}
