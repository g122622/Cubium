...
TEST_F(StainedGlassBlockTest, ColorComponents_MatchBeaconColors) {
    ...
    for (u8 i = 0; i < static_cast<u8>(DyeColor::Count); ++i) {
        ...
        constexpr f32 EPSILON = 0.0001f;
        EXPECT_NEAR((*blockColor)[0], expectedColor[0], EPSILON)
            << "R mismatch for color " << i;
        EXPECT_NEAR((*blockColor)[1], expectedColor[1], EPSILON)
            << "G mismatch for color " << i;
        EXPECT_NEAR((*blockColor)[2], expectedColor[2], EPSILON)
            << "B mismatch for color " << i;
    }
}
