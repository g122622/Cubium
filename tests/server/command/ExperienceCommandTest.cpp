 ...
 * @brief 测试服务器，用于命令测试。
 */
class ExperienceTestServer final : public server::IServer {
public:
    ExperienceTestServer()
...
protected:
    ...
    {
        // 注册命令
        ExperienceCommand::registerTo(m_server.commandRegistry().dispatcher());
    }
...
{
    // 测试省略 points/levels 时默认为 points
...
{
    // set 命令不允许负值（IntegerArgumentType::integer(0)）
...
{
    // 测试省略 points/levels 时默认为 levels
