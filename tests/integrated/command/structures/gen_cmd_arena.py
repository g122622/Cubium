...
cmd_arena：9×7×9 石盒，内部 7×5×7 空气腔，专供命令类（/setblock /fill /summon 等）测试。
- y=0：满铺 stone 地板（实体站立支撑）。
- y=1..5：外围 stone 墙 + 内部 7×5×7 空气腔（x,z∈[1,7], y∈[1,5]）。
- y=6：满铺 stone 封顶（命令测试不依赖光照，封顶隔离外部 worldgen 干扰）。
玩家放空气腔内执行命令，命令目标坐标在空气腔内，helper 相对坐标清晰。
...
# ===================== 结构定义 =====================
SX, SY, SZ = 9, 7, 9  # cmd_arena: 9×7×9
...
def build_block(x, y, z):
    """返回 (x,y,z) 处的调色板索引。cmd_arena 石盒布局。"""
    # y=0：满铺 stone 地板
    if y == 0:
        return 0  # stone
    # y=6：满铺 stone 封顶
...
def main():
    ...
    # cmd_arena 无方块实体，写空 compound {}。缺此字段基岩解析报 "block_position_data field, a required
    ...
    fn = "tests/integrated/command/structures/gametests/cmd_arena.mcstructure"
    if len(sys.argv) > 1:
        fn = sys.argv[1]
