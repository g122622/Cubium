...
// （canAttachTo(Down) 为 false）则返回 air 自毁而非死珊瑚。故珊瑚扇必须放在固体上方。glass_pit y=1
// 下方 y=0 是 glass（玻璃底），满足支撑（glass canAttachTo 为 true）。
//
...
function coralFanDiesWhenNoWaterNearby(test: Test): void {
  // 放管状珊瑚扇 (3,1,1)，下方 (3,0,1) 为 glass_pit 玻璃底支撑（CoralFanBlock 要求下方固体，否则自毁）。
