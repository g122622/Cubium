...
// 专有链式方法：
//   - skyAccess（对齐 Java GameTest TestData 字段，类型声明见 cubium-gametest-augment.d.ts）
//   - loadSpawnChunks（项目独有，强制加载结构周围 SPAWN_DISTANCE_CHUNK(8) 区块，供 NaturalSpawner 测试）
// 官方基岩 BDS 的 RegistrationBuilder 没有这两个方法，测试代码调用 .skyAccess(true) /
// .loadSpawnChunks(true) 时抛 TypeError: not a function。该异常发生在 main.js 顶层 register 链式
// 阶段，会导致整个行为包加载失败、所有测试都无法注册。
//
// 注意：setupTicks 是基岩 RegistrationBuilder 原生方法（见 index.d.ts），无需降级；Cubium 也支持。
// 仅 skyAccess / loadSpawnChunks 是 Cubium 专有，需在基岩侧降级为 no-op。
//
// 修复策略（双保险，按可靠性排序）：
// 1. RegistrationBuilder.prototype 注入：基岩 RegistrationBuilder 是导出 class，prototype 可扩展。
//    在 prototype 上注入专有方法 no-op（返回 this 保持链式）。基岩实例自身无该方法，沿原型链
//    命中注入的 no-op；Cubium 实例自身有 C++ 绑定的实现，覆盖 prototype，无副作用。
//    此方案不替换 register，最可靠。
// 2. register Proxy 包装（备用）：若 prototype 注入失败（如基岩 prototype 冻结），退而用 Proxy 包装
//    GameTest.register 返回的 builder，对专有方法降级 no-op。基岩 ESM namespace 属性可能 read-only，
 ...
 * - 基岩侧：实例自身无 skyAccess/loadSpawnChunks，沿原型链命中此 no-op，链式不断。
 * - Cubium 侧：实例自身有 C++ 绑定的实现，原型方法被覆盖，无副作用。
