...
dark_cavern：41×7×9 露天石地围栏，专供 NaturalSpawner 自然生成（怪物）测试。
- y=0：满铺 stone 地板（生成位下方 solid 支撑，满足 OnGround 放置检查）。
- y=1..3：外围 stone 墙 + 内部 39×3×7 空气腔（生成区，x∈[1,39], y∈[1,3], z∈[1,7]）。
- y=4..6：内部全 air 无封顶（露天）；外围 stone 墙延伸至 y=6 围栏防怪物寻路走出。
- 四壁 stone 墙（x=0/x=40, z=0/z=8，y=1..6）围栏。
玩家放 x=2 端（距生成腔 x≈38 约 36 格，满足 NaturalSpawner 24-128 格距离门控）。

关键设计：露天 + night batch + skyAccess（必须配合测试注册的 .skyAccess(true)）。
- NaturalSpawner._getSpawnHeight 用 heightmap（MotionBlockingNoLeaves）选 y：取该列最高实心方块
  上方第一格 air。结构埋 gridStartY=-59 地下，须 skyAccess(true) 清空结构上方 worldgen 石头，使
  heightmap 落在结构地板 stone（文件 y=0，世界 y=-58）→ spawnY=文件 y=1 air（结构内生成区）。若不
  skyAccess，heightmap 落 worldgen 地表（~y=63），spawnY 在地表 air（结构外），怪物生成在区域外不被计数。
- 怪物光照门槛（_checkLightLevel 对齐 vanilla Monster.isDarkEnoughToSpawn）：用 getMaxLocalRawBrightness
  （含 skyDarkening 时间衰减）。night batch dayTime=18000 夜晚 skyDarkening 使露天 skyLight 衰减到 ~4
  <=7 通过 → 怪物在夜晚露天结构内生成。day batch 白天 skyDarkening=0，露天 skyLight=15>7 拒绝。
- night batch 避免亡灵白天燃烧（夜晚亡灵不燃，生成后存活）。
- 不封顶：封顶 stone 会使 heightmap 落封顶（文件 y=6），spawnY=封顶上方 air（结构外露天），怪物生成在
  区域外。故必须露天无封顶，让 heightmap 落地板，spawnY 落结构内 air 腔。
...
# ===================== 结构定义 =====================
SX, SY, SZ = 41, 7, 9  # dark_cavern: 41×7×9
...
def build_block(x, y, z):
    """返回 (x,y,z) 处的调色板索引。dark_cavern 露天石地围栏布局。"""
    # y=0：满铺 stone 地板
    if y == 0:
        return 0  # stone
    # y=1..3：外围 stone 墙 + 内部 air（生成区）
    if y <= 3 and (x == 0 or x == SX - 1 or z == 0 or z == SZ - 1):
        return 0  # stone 墙
    # y=4..6 外围 stone 墙延伸（围栏防怪物寻路走出，y=1..6 全高墙）
    if x == 0 or x == SX - 1 or z == 0 or z == SZ - 1:
        return 0  # stone 墙
    # 内部 y=1..6 全 air（无封顶露天，heightmap 落地板使 spawnY 在结构内 air 腔）
    return 1  # air
...
def main():
    ...
    # dark_cavern 无方块实体，写空 compound {}。缺此字段基岩解析报 "block_position_data field, a required
    ...
    fn = "tests/integrated/mob_behavior/structures/gametests/dark_cavern.mcstructure"
    if len(sys.argv) > 1:
        fn = sys.argv[1]
