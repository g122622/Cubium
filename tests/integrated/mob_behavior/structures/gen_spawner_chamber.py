...
spawner_chamber：11×7×11 封顶遮光石盒，内部 9×5×9 空气腔，专供刷怪箱（mob_spawner）测试。
- y=0：满铺 stone 地板（生成位下方 solid 支撑，满足 OnGround 放置检查）。
- y=1..5：外围 stone 墙 + 内部 9×5×9 空气腔（x,z∈[1,9], y∈[1,5]）。
- y=6：满铺 stone 封顶遮光（结构内 skyLight=0，满足怪物刷怪箱 isDarkEnoughToSpawn）。
...
# ===================== 结构定义 =====================
SX, SY, SZ = 11, 7, 11  # spawner_chamber: 11×7×11
...
def build_block(x, y, z):
    """返回 (x,y,z) 处的调色板索引。spawner_chamber 封顶遮光石盒布局。"""
    # y=0：满铺 stone 地板
    if y == 0:
        return 0  # stone
    # y=6：满铺 stone 封顶遮光
...
def main():
    ...
    # spawner_chamber 无方块实体，写空 compound {}。缺此字段基岩解析报 "block_position_data field, a required
    ...
    fn = "tests/integrated/mob_behavior/structures/gametests/spawner_chamber.mcstructure"
    if len(sys.argv) > 1:
        fn = sys.argv[1]
