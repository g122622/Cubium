...
ghast_arena：15×30×15 恶魂火球测试专用高耸玻璃竞技场。
- y=0：满铺 cobblestone 圆石地板（恶魂 4×4×4 落地支撑）。
- y=1..28：外围 glass 墙 + 内部 13×28×13 空气腔（恶魂漂浮空间）。
- y=29：满铺 cobblestone 圆石天花板（封顶遮光，模拟下界洞穴黑暗，防自然生成干扰）。

为何需要 30 格高：恶魂 4 格高 + GhastRandomFlyGoal WANDER_RANGE=16 随机漂浮，
setNoGravity 简化实现无飞行摩擦衰减，velocity 累积致恶魂持续上升最多 ~16 格。
mediumglass(9 格高) 恶魂漂 3 格即撞天花板，火球从恶魂眼睛生成立即撞天花板 onBlockHit
消失，pollUntilSucceed 查不到。30 格高给恶魂 25 格漂浮空间，火球水平飞向玩家不撞顶。
...
# ===================== 结构定义 =====================
SX, SY, SZ = 15, 30, 15  # ghast_arena: 15×30×15

# 调色板：索引 → {方块名, states}
# 0=cobblestone, 1=glass, 2=air
PALETTE = [
...
def build_block(x, y, z):
    """返回 (x,y,z) 处的调色板索引。ghast_arena 高耸玻璃竞技场布局。"""
    # y=0：满铺 cobblestone 圆石地板
    if y == 0:
        return 0  # cobblestone
    # y=29：满铺 cobblestone 圆石天花板（封顶遮光）
    if y == SY - 1:
        return 0  # cobblestone
    # 四壁 glass 墙（y=1..28，x/z 边界）
...
def main():
    ...
    # ghast_arena 无方块实体，写空 compound {}。缺此字段基岩解析报 "block_position_data field, a required
    # field, is missing"（虽容错继续，但为正确性补齐）。
    default_palette = {
    ...
    fn = "tests/integrated/mob_behavior/structures/gametests/ghast_arena.mcstructure"
    if len(sys.argv) > 1:
        fn = sys.argv[1]
