...
open_grass_hall：41×7×9 露天草地长廊，专供 NaturalSpawner 自然生成（动物）测试。
- y=0：满铺 grass_block 草地地板（动物生成需草地方块支撑 + 光照>7）。
- y=1..3：外围 glass 墙 + 内部 39×3×7 空气腔（生成区）。
- y=4..6：全 air 无封顶（露天，skyLight=15 满足动物 skyLight>7 门槛）。
- 四壁 glass（x=0/x=40, z=0/z=8，y=1..6）透光围栏防动物寻路走出。
玩家放 x=2 端（距生成腔 x≈38 约 36 格，满足 NaturalSpawner 24-128 格距离门控）。

关键：露天 skyLight=15（不减 skyDarkening），满足动物 max(skyLight,blockLight)>7 门槛。
怪物需 skyLight<=7，露天 15 拒绝——故露天只生成动物，不生成怪物（天然隔离）。
...
# ===================== 结构定义 =====================
SX, SY, SZ = 41, 7, 9  # open_grass_hall: 41×7×9
...
def build_block(x, y, z):
    """返回 (x,y,z) 处的调色板索引。open_grass_hall 露天草地长廊布局。"""
    # y=0：满铺 grass_block 草地地板
    if y == 0:
        return 0  # grass_block
    # 四壁 glass 墙（y=1..6，x/z 边界）
...
def main():
    ...
    # open_grass_hall 无方块实体，写空 compound {}。缺此字段基岩解析报 "block_position_data field, a required
    ...
    fn = "tests/integrated/mob_behavior/structures/gametests/open_grass_hall.mcstructure"
    if len(sys.argv) > 1:
        fn = sys.argv[1]
