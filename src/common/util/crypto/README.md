# Crypto Module

加密和哈希工具库，提供密码学相关的实用功能。

## 目录结构

```
crypto/
├── Sha256.hpp             # SHA-256 哈希算法（用于世界种子哈希）
├── Sha256.cpp             # SHA-256 实现
├── Sha1.hpp               # SHA-1 哈希算法（用于皮肤缓存键生成）
├── Sha1.cpp               # SHA-1 实现
├── Md5.hpp                # MD5 哈希算法（用于离线UUID生成）
├── Md5.cpp                # MD5 实现
└── README.md              # 本文档
```

## 内部模块关系

```
┌─────────────────────────────────────────────────────────┐
│                     crypto 模块                          │
├─────────────────────────────────────────────────────────┤
│  Sha256（FIPS 180-4）   Sha1（FIPS 180-4）   Md5（RFC 1321）│
│  - 世界种子 hashedSeed   - 皮肤缓存键生成     - 离线模式 UUID  │
│  - 通用哈希计算          - 通用哈希计算       - 通用哈希计算    │
└─────────────────────────────────────────────────────────┘
```

三个模块相互独立，无内部依赖。

## 上下游外部依赖关系

### 上游依赖（本模块依赖的）

| 依赖 | 用途 |
|------|------|
| `common/core/Types.hpp` | 基础类型定义（u8, u32, u64 等） |
| C++ 标准库 | `<array>`, `<span>`, `<string>`, `<vector>` |

### 下游依赖（依赖本模块的）

| 模块 | 使用 |
|------|------|
| 网络协议层 | `Sha256::hashWorldSeed` 用于维度切换包 |
| 玩家系统 | `Md5::hash` 用于离线模式 UUID 生成 |
| 皮肤系统 | `Sha1::hash` + `Sha1::toHexString` 用于缓存键生成 |

## 容易踩的坑

### 1. MD5 和 SHA-1 安全性问题

MD5 已被证明不安全，SHA-1 也已被证明存在碰撞攻击漏洞。两者**仅用于兼容性需求**（如 Minecraft 离线 UUID 生成、皮肤缓存键生成），切勿用于密码存储或安全验证。

### 2. hashWorldSeed 字节序

`Sha256::hashWorldSeed` 的实现遵循 Guava 的 `Hashing.sha256().hashLong(seed).asLong()`：
- 输入：种子以**大端序**转换为 8 字节
- 输出：哈希前 8 字节以**小端序**解释为 u64

输入输出字节序不同，这是 Minecraft 协议的标准行为。

### 3. 与 Java 版一致性

`hashWorldSeed` 结果必须与 Java 版 `BiomeManager.func_235200_a_` 完全一致，修改此函数需同步验证跨版本兼容性。

### 4. 十六进制输出格式

`toHexString` 输出小写十六进制字符串，如需大写需自行转换。

### 5. SHA-1 字节序

SHA-1 使用大端序处理消息字和输出摘要，这与 SHA-256 一致，但与 MD5（小端序）不同。
