    ...
    do {                                                                        \
        if (!(expr)) {                                                          \
            ::mc::assert::AssertManager::instance().handleFailure(             \
                #expr, msg, __FILE__, __LINE__, __func__, level                \
            );                                                                  \
        }                                                                       \
    } while (false)
...
#define MC_ASSERT_EQ(a, b)                                                      \
    do {                                                                        \
        ...
        if (!(_a == _b)) {                                                      \
            ::mc::assert::AssertManager::instance().handleFailure(             \
                #a " == " #b,                                                   \
                ::mc::assert::detail::formatComparisonMessage(                  \
                    "not equal", #a, _a, #b, _b                                 \
                ).c_str(),                                                      \
    ...
    do {                                                                        \
        ...
        if (_a == _b) {                                                         \
            ::mc::assert::AssertManager::instance().handleFailure(             \
                #a " != " #b,                                                   \
                ::mc::assert::detail::formatComparisonMessage(                  \
                    "equal (should be different)", #a, _a, #b, _b               \
                ).c_str(),                                                      \
                __FILE__, __LINE__, __func__,                                   \
                ::mc::assert::AssertLevel::Debug                                \
            );                                                                  \
    ...
    do {                                                                        \
        ...
        if (!(_a <= _b)) {                                                      \
            ::mc::assert::AssertManager::instance().handleFailure(             \
                #a " <= " #b,                                                   \
                ::mc::assert::detail::formatComparisonMessage(                  \
                    "not less than or equal", #a, _a, #b, _b                    \
                ).c_str(),                                                      \
                __FILE__, __LINE__, __func__,                                   \
                ::mc::assert::AssertLevel::Debug                                \
            );                                                                  \
    ...
    do {                                                                        \
        ...
        if (!(_a >= _b)) {                                                      \
            ::mc::assert::AssertManager::instance().handleFailure(             \
                #a " >= " #b,                                                   \
                ::mc::assert::detail::formatComparisonMessage(                  \
                    "not greater than or equal", #a, _a, #b, _b                 \
                ).c_str(),                                                      \
                __FILE__, __LINE__, __func__,                                   \
                ::mc::assert::AssertLevel::Debug                                \
            );                                                                  \
...
#define MC_ASSERT_UNREACHABLE()                                                 \
    ::mc::assert::AssertManager::instance().handleFailure(                      \
        "unreachable code", nullptr, __FILE__, __LINE__, __func__,              \
        ::mc::assert::AssertLevel::Fatal                                        \
    )
...
#define MC_ASSERT_UNREACHABLE_MSG(msg)                                          \
    ::mc::assert::AssertManager::instance().handleFailure(                      \
        "unreachable code", msg, __FILE__, __LINE__, __func__,                  \
        ::mc::assert::AssertLevel::Fatal                                        \
    )
...
#define MC_ASSERT_FAIL(msg)                                                     \
    ::mc::assert::AssertManager::instance().handleFailure(                      \
        "assertion failed", msg, __FILE__, __LINE__, __func__,                  \
        ::mc::assert::AssertLevel::Fatal                                        \
    )
...
#define MC_ASSERT_NOT_IMPLEMENTED()                                             \
    ::mc::assert::AssertManager::instance().handleFailure(                      \
        "not implemented", __func__, __FILE__, __LINE__, __func__,              \
        ::mc::assert::AssertLevel::Fatal                                        \
    )
 ...
 */
#define MC_ASSERT_CAST(ptr, Type)                                               \
    MC_ASSERT_NOT_NULL(ptr)
