...
Result<std::vector<u8>> PlaySoundPacket::serialize() const {
    ...
    // 写入位置（定点整数）
    serializer.writeInt32(m_x);
    serializer.writeInt32(m_y);
    serializer.writeInt32(m_z);

    // 写入音量和音调
    serializer.writeFloat(m_volume);
    serializer.writeFloat(m_pitch);

    return serializer.toPacket(*this);
}
...
Result<void> PlaySoundPacket::deserialize(const u8* data, size_t size) {
    ...
    // 读取位置
    auto xResult = deserializer.readInt32();
    if (!xResult.success()) {
        return xResult.error();
    }
    m_x = xResult.value();

    auto yResult = deserializer.readInt32();
    if (!yResult.success()) {
        return yResult.error();
    }
    m_y = yResult.value();

    auto zResult = deserializer.readInt32();
    if (!zResult.success()) {
    ...
    // 读取音量和音调
    auto volumeResult = deserializer.readFloat();
    if (!volumeResult.success()) {
        return volumeResult.error();
    }
    m_volume = volumeResult.value();

    auto pitchResult = deserializer.readFloat();
...
Result<void> StopSoundPacket::deserialize(const u8* data, size_t size) {
    ...
    // 读取标志字节
    auto flagsResult = deserializer.readUInt8();
