...
private:
     ...
     */
    [[nodiscard]] ScriptResult exceptionToResult();

    /**
     * @brief 将JSValue转换为ScriptValue
     */
    [[nodiscard]] ScriptValue jsValueToScriptValue(JSValue val);

    /**
     * @brief 将ScriptValue转换为JSValue
     */
    [[nodiscard]] JSValue scriptValueToJSValue(const ScriptValue& val);
};
