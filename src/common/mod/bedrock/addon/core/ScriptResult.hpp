...
class ScriptValue {
public:
    ScriptValue() : m_type(ScriptValueType::Undefined) {}
    explicit ScriptValue(std::nullptr_t) : m_type(ScriptValueType::Null) {}
    explicit ScriptValue(bool val) : m_type(ScriptValueType::Boolean), m_data(val) {}
    explicit ScriptValue(f64 val) : m_type(ScriptValueType::Number), m_data(val) {}
    explicit ScriptValue(i32 val) : m_type(ScriptValueType::Number), m_data(static_cast<f64>(val)) {}
    explicit ScriptValue(std::string val)
        : m_type(ScriptValueType::String), m_data(std::move(val)) {}
...
class ScriptResult {
public:
    static ScriptResult ok(ScriptValue value = ScriptValue()) {
        return ScriptResult(std::move(value), true, "");
    }

    static ScriptResult error(std::string message) {
        return ScriptResult(ScriptValue(), false, std::move(message));
    }
...
private:
    ScriptResult(ScriptValue value, bool success, std::string errorMessage)
        : m_value(std::move(value)), m_success(success), m_errorMessage(std::move(errorMessage)) {}
