...
const Food Foods::CHICKEN = Food(2, 0.3f)
    .setMeat()
    .addEffect(EffectType::Hunger, 600, 0, 0.3f);  // 30%概率饥饿 30秒
...
const Food Foods::POISONOUS_POTATO = Food(2, 0.3f)
    .addEffect(EffectType::Poison, 100, 0, 0.6f);  // 60%概率中毒 5秒
...
const Food Foods::ROTTEN_FLESH = Food(4, 0.1f)
    .setMeat()
    .addEffect(EffectType::Hunger, 600, 0, 0.8f);  // 80%概率饥饿 30秒

const Food Foods::SALMON(2, 0.1f);

const Food Foods::SPIDER_EYE = Food(2, 0.8f)
    .addEffect(EffectType::Poison, 100, 0, 1.0f);  // 100%概率中毒 5秒
