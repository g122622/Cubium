...
风爆附魔在 `Player::_applyWindBurstEffect()` 中实现，采用与 `WindChargeEntity::applyWindBurst()` 相同的 TRIGGER 爆炸模式：不造成伤害、不破坏方块，仅施加定向击退（含爆炸保护减免）、播放音效和粒子。爆炸参数（半径 3.5、击退乘数随等级 1.2/1.75/2.2）与 MC Java 的 `ExplodeEffect` 注册数据一致。
