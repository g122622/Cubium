...
19. **BrushItem 刷扫机制**（对齐 MC 1.21.11 `BrushItem`）：
    ...
    - **射线检测**：每次刷扫触发 tick 通过 `calculateHitResult` 从玩家眼睛位置沿视线方向做方块射线检测，最大距离 `BLOCK_INTERACTION_RANGE(5.0)`；未命中时调用 `stopActiveHand()` 取消使用（对齐 MC `releaseUsingItem`）
    ...
    - **耐久消耗**：当前刷扫仅播放音效和粒子，不消耗耐久、不刷出物品。TODO: 待 `BrushableBlockEntity` 实现后，在 `onUseTick` 中调用 `brushableblockentity.brush(gameTime, serverlevel, player, direction, stack)`，返回 true 时消耗1耐久、播放完成音效、触发 `BRUSH_BLOCK_COMPLETE` 世界事件
    ...
    - **BLOCK_INTERACTION_RANGE 常量**：`BrushItem.hpp` 中以 `static constexpr f32 BLOCK_INTERACTION_RANGE = 5.0f` 内联。TODO: 待 `Player::blockInteractionRange()` 实现后改为调用该方法
