...
├── AxeItem.hpp/cpp            # 斧类工具（含去皮、刮削、除蜡功能）
...
- `src/common/world/block/blocks/copper/IOxidizableBlock.hpp` - 可氧化方块接口（AxeItem 刮削依赖 getPreviousOxidationBlock 反向链）
...
AxeItem 除蜡（wax-off）时仅调用 `world.playEvent(WorldEvents::WAX_OFF, pos, 0)` 即可，**不需要**额外调用 `playSound`。`WAX_OFF` 事件（ID 3004）本身已包含音效和粒子效果，额外调用 `playSound` 会导致双重音效。这与去皮（stripping）不同——去皮仅播放 `playSound(ITEM_AXE_STRIP)` 而不触发 levelEvent。刮削（scraping）与除蜡类似，仅调用 `world.playEvent(WorldEvents::SCRAPE, pos, 0)`，该事件（ID 3005）已包含音效和粒子效果。参见 HoneycombItem::onItemUse 中 WAX_ON 的处理方式（仅调用 playEvent）。

### 8. AxeItem onItemUse 处理顺序

MC 原版 AxeItem::useOn 的处理顺序为：1.去皮（strip）→ 2.刮削（scrape/去氧化）→ 3.除蜡（wax-off）。每个步骤独立检查，只有第一个匹配的步骤被执行（fallthrough if-empty 模式）。刮削通过 `IOxidizableBlock::getPreviousOxidationBlock()` 访问反向氧化链，将铜方块降级到上一氧化等级。涂蜡铜方块不实现 `IOxidizableBlock`，因此刮削步骤不会匹配涂蜡方块，会正确 fallthrough 到除蜡步骤。
