...
│   │   ├── MemoryModuleType.hpp / cpp #记忆模块类型（46 +
        种）
...
```
                ...
                ** 数据流**： 1. `GoalSelector` 按 tick 调度，选择当前应执行的目标 2. 目标通过 `Controller`（Look
            / Move /
            Jump）控制实体行为 3. 移动类目标通过 `PathNavigator` 计算和执行路径
    ...
    | `entity / attribute` | `Attributes` - 移动速度、跟随范围等属性 | | `world` | `IWorld` - 世界查询接口 |
    | `util / math` | `Random` - 随机数生成、`Vector3` - 位置计算 |

    ## #下游依赖（依赖本模块的外部模块）

    | 模块 | 使用方式 | | -- -- --| -- -- -- -- --| | `entity / entities / passive/*` | 各种动物使用 Goal 系统（猪、牛、羊等） |
...
**状态**：Brain 框架已完成基础结构，action 任务（AttackTask、BreedTask、PlayDeadTask、JumpTask、KickTask）已实现，movement 和 interact 任务也已实现。EatTask 因饥饿系统未实现暂为占位。当前仅 VillagerEntity 集成了 Brain 系统，动物实体仍使用 Goal 系统。当 AnimalEntity 子类迁移到 Brain 系统后，需在对应实体的 `initializeBrain()` 中注册任务和传感器。
