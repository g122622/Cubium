 ...
 * 对齐基岩版 PortalCooldownDurationComponent（mc/entity/components/PortalCooldownDurationComponent.h）
 * ——基岩版将冷却单独组件化；项目把传送门全套状态并入同一组件，因四字段同属传送门
 * 时序且总是相伴读写（onEntityCollision 设 inPortal/portalPos，tickPortal 读之递进，
 * triggerPortalCooldown 设 cooldown）。
 *
 * 字段语义：
 * - m_portalCooldown：传送冷却计数。baseTick 每帧 -1（移入 PortalTickSystem 后由系统递减），
