...
public:
	...
	[[nodiscard]] bool canFitPassenger() const {
		return getPassengers().size() < MAX_PASSENGERS && m_status != BoatStatus::UnderWater;
	}
...
private:
	...
	static constexpr f32 PADDLE_SPEED = 0.05f;
	static constexpr f32 WATER_SPEED_MULT = 0.04f;
	static constexpr f32 LAND_FRICTION = 0.5f;
	static constexpr f32 WATER_FRICTION = 0.9f;
	static constexpr f32 GRAVITY = 0.04f;
	static constexpr f32 BUOYANCY = 0.06153846f;
	static constexpr i32 MAX_PASSENGERS = 2;
	static constexpr f32 DAMAGE_THRESHOLD = 40.0f;
	static constexpr i32 TIME_SINCE_HIT_DECAY = 10;
};
