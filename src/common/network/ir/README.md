...
└── packets/
    ...
    └── play/PlayPackets.hpp                # KeepAlive/Disconnect/MovePlayerPos/Chat（在用包子集，Phase3 补全）
```
...
4. **阶段变体与 addPacket 顺序/altIndex 强耦合**：阶段变体里备选项顺序 = ProtocolInfoBuilder::addPacket 的 altIndex = wire packet id；新增包 struct 须同时登记到变体与包表，顺序一致。
