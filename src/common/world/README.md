...
- `PlayerInteractedWithEntityTrigger` 不通过 IWorld 回调触发，而是在 `ServerPlayHandler::_triggerPlayerInteractedWithEntity`（INTERACT/INTERACT_AT 返回 Success 时调用）中按 `AbstractCriterionTrigger::trigger()` 模板触发。
