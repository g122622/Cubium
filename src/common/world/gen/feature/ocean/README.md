...
所有特征类均采用 `XxxFeature` + `ConfiguredXxxFeature` + `XxxFeatures` 工厂的三层结构：
- `XxxFeature` - 核心放置逻辑
- `ConfiguredXxxFeature` - 实现 `ConfiguredFeatureBase` 接口，供 `FeatureRegistry` 注册
- `XxxFeatures` - 工厂类，提供预定义配置的静态创建方法
...
**被依赖方（下游）：**
- `FeatureRegistry` - 注册所有 `ConfiguredXxxFeature`
- `BiomeGenerationSettings` - 生物群系生成设置引用对应 FeatureIds
- FeatureIds 常量组：`KelpFeatureIds`、`SeagrassFeatureIds`、`SeaPickleFeatureIds`、`CoralFeatureIds`、`OceanDecorationFeatureIds`、`BlueIceFeatureIds`
...
- **温区变体同步**：海带与海草已拆分冷/暖/深海/河流/沼泽等变体，更新 `FeatureIds` 后必须同步 `FeatureRegistry`、`BiomeGenerationSettings`。
