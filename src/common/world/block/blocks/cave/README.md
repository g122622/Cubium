...
├── CaveVinesBlock.hpp / cpp #洞穴藤蔓（发光浆果，顶部）
├── CaveVinesPlantBlock.hpp / cpp #洞穴藤蔓植物（发光浆果，中部 / 底部）
...
```
    ...
    | 模块 | 用途 | | -- -- --| -- -- --| | `world / block / Block` | 方块基类 | | `world / block / IWaterLoggable` |
    含水方块接口 | | `world / block / IBonemealable` | 骨粉可催熟接口 | | `world / block / Material` | 材质系统 |
    | `world / block / WaterLoggableHelpers` | 含水工具函数 | | `world / block / BlockStateProperties` |
    方块状态属性（NORTH / SOUTH / EAST / WEST / UP / DOWN / TIP等） | | `physics / collision / CollisionShape` |
    碰撞形状 | | `util / Direction` | 方向枚举 |

    ## #下游依赖（谁依赖本模块）

    | 模块 | 用途 | | -- -- --| -- -- --| | `world / block / registry / CaveBlocks` | 注册所有洞穴方块 |
...
`GlowLichenBlock` 预计算了64种形状组合（2 ^
        6 = NORTH | SOUTH | EAST | WEST | UP |
    DOWN），每个面方向是一个1像素厚的薄板。通过 `_getShapeIndex` 位编码索引从 `m_shapes` 数组查找。当没有任何面激活时返回 `fullBlock` 形状。
...
`SporeBlossomBlock::
        isValidPosition()` 检查上方方块的向下实心面（`isSolidSide`），且当前位置不能在水中。支撑失效时直接在 `updatePostPlacement` 中返回空气状态。

    ## #4. BuddingAmethystBlock 使用 ResourceLocation 查找方块

        由于注册顺序依赖，`BuddingAmethystBlock` 通过 `BlockRegistry::instance()
            .getBlock(ResourceLocation(...))` 延迟查找紫水晶簇方块，而非直接引用指针。
