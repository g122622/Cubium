...
- **调用路径**：`BlockInteractionManager::handleBlockBreak` → `spawnAfterBreak(tool, true)` → `InfestedBlock::spawnAfterBreak`；爆炸路径 `Explosion::_destroyBlocks` → `spawnAfterBreak(nullptr, false)`；实体破坏路径（末影龙/凋灵/掠夺者/蠹虫苏醒/活塞/流体/命令）均调用 `spawnAfterBreak(nullptr, false)`
