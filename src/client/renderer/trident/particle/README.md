...
- **NautilusParticle**: 鹦鹉螺粒子
  - 发光粒子，最大亮度（15728880 = blockLight 15 + skyLight 15）
  - 无重力，无物理碰撞
  - 向目标方向移动，速度逐渐衰减（摩擦力 0.95）
  - 生命周期约 60 tick（约 3 秒）
  - 缩放动画：前期变大（0-30%），中期保持（30-70%），后期缩小（70-100%）
  - 旋转效果：每 tick 增加 0.05 弧度
  - 淡出效果：生命周期 > 50% 时 alpha 逐渐降低
  - 颜色：白色/淡蓝色色调（R/G: 0.8-1.0, B: 1.0）
