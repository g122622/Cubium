...
动画纹理需要特殊的 tick/upload 生命周期管理，确保动画帧正确更新并同步到 GPU：
...
│  主线程 tick (20 TPS)                                           │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ tickTextureAnimations(deltaTime)                        │   │
│  │ ├─ 遍历所有 AnimatedSprite                              │   │
│  │ ├─ 更新帧计时器                                         │   │
│  │ └─ 检测帧切换，标记需要更新                              │   │
│  └─────────────────────────────────────────────────────────┘   │
...
class TridentEngine {
public:
    // 在主线程 tick 中调用，更新动画状态
    void tickTextureAnimations(f32 deltaTime);
    
    // 在渲染帧中调用，上传更新的动画帧到 GPU
    void uploadAnimationFrames(VkCommandBuffer cmd);
    
private:
    std::unique_ptr<TextureAtlasTicker> m_atlasTicker;
};
```
...
1. **增量更新**：只上传帧变化的纹理区域，而非整个图集
2. **批量上传**：多个动画更新合并到单次命令缓冲区
3. **双缓冲**：避免渲染时更新正在使用的纹理
