...
2. **模板类显式实例化**：`SaddleLayer`, `SheepWoolLayer`, `ArrowLayer`, `HeldBlockLayer` 均为模板类，使用时需要显式实例化（如 `template class HeldBlockLayer<::mc::EndermanEntity>;`）。

3. **编译时类型检查**：`HeldBlockLayer` 和 `SheepWoolLayer` 使用 `if constexpr` + `std::is_base_of_v` 进行编译时类型检查，确保只有特定实体类型才执行相关逻辑。

4. **VillagerLayer 静态网格缓存**：使用静态 `std::unordered_map` 缓存网格，配合 `std::shared_mutex` 实现线程安全。务必在渲染前调用 `setTextureAtlas()` 设置纹理图集，否则 UV 重映射不会生效。

5. **网格缓存生命周期**：各渲染器使用 `static std::unique_ptr<pipeline::EntityMesh>` 缓存网格，在整个程序生命周期内有效。首次渲染时会创建网格，后续复用。
...
- MC 1.16.5 SaddleLayer / SheepWoolLayer / WolfCollarLayer
- MC 1.16.5 StuckInBodyLayer（箭矢附着）
- MC 1.16.5 HeldBlockLayer（末影人手持方块）
- MC 1.16.5 VillagerLevelPendantLayer（村民多层纹理）
