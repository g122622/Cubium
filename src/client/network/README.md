...
`Commands`/`Explosion`/`MapItemData`/`LevelParticles`/`UpdateRecipes` 等 IR 字段为 opaque `vector<u8>`（透传旧序列化字节），未对齐 1.21.11 codec，标 `TODO(Phase6)`。新接入的包类型应在 `ClientPlayVisitor` 加分支，不要回退到旧 packet 反序列化。
