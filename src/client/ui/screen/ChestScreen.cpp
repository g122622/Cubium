...
ChestScreen::ChestScreen(ContainerId containerId,
    ...
    : AbstractContainerScreen<mc::blockentity::ChestContainer>(
          std::make_unique<mc::blockentity::ChestContainer>(
              containerId,
              playerInventory,
              std::shared_ptr<mc::IInventory>(
                  std::make_shared<mc::blockentity::SimpleInventory>(rows * mc::blockentity::ChestContainer::SLOTS_PER_ROW)),
              rows),
...
void ChestScreen::renderContainerBackground()
{
    ...
    m_gui->fillRect(static_cast<f32>(m_leftPos), static_cast<f32>(m_topPos),
                    static_cast<f32>(GUI_WIDTH), static_cast<f32>(getImageHeight()), BG_COLOR);
    m_gui->drawRect(static_cast<f32>(m_leftPos), static_cast<f32>(m_topPos),
                    static_cast<f32>(GUI_WIDTH), static_cast<f32>(getImageHeight()), BORDER_COLOR);
}
