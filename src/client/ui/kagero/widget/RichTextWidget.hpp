...
public:
    ...
    void paint(PaintContext& ctx) override {
        ...
        for (const auto& line : m_lines) {
            ...
            for (const auto& run : line.runs) {
                // 构建渲染样式
                u32 color = run.style.getColor().has_value()
    ...
    void setWordWrap(bool wrap) { m_wordWrap = wrap; m_linesDirty = true; }
    void setAlignment(TextAlignment alignment) { m_alignment = alignment; }
    void setLineSpacing(f32 spacing) { m_lineSpacing = spacing; m_linesDirty = true; }
    void setMaxLines(i32 maxLines) { m_maxLines = maxLines; m_linesDirty = true; }
    void setFont(void* font) { m_font = font; m_linesDirty = true; }
...
private:
     ...
     */
    void flattenComponent(const text::ITextComponent& component,
                          const text::Style& parentStyle,
                          std::vector<TextRun>& runs) {
        // 合并样式
        text::Style style = component.getStyle().mergeWithParent(parentStyle);

        // 获取文本
        const text::StringTextComponent* stringComp =
