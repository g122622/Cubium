...
void OpenALSource::queueBuffers(std::span<AudioBufferId> buffers) {
    if (!isValid() || buffers.empty()) {
        return;
    }

    // 收集 OpenAL buffer handles
    std::vector<ALuint> alBuffers;
    alBuffers.reserve(buffers.size());
...
u32 OpenALSource::unqueueBuffers(std::span<AudioBufferId> buffers) {
    if (!isValid() || buffers.empty()) {
        return 0;
    }
    ...
    alSourceUnqueueBuffers(m_source, static_cast<ALsizei>(buffers.size()), alBuffers.data());
    ...
    // 这是一个简化实现，需要实际的映射
    processed = static_cast<ALsizei>(buffers.size());
...
Result<std::unique_ptr<OpenALBuffer>> OpenALBuffer::create(
    ...
    // 获取 OpenAL 格式
    ALenum format = getALFormat(data.format);
    if (format == 0) {
