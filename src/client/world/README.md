...
- **`deserializeChunk` 在 worker 异步执行（零拷贝视图下沉）**：`onChunkData` 接收的是原始网络缓冲的**视图**（`const u8*` + `size`）而非拷贝，外加 `shared_ptr<vector<u8>>` 保活句柄。`onChunkData` 把视图 + 保活句柄 move 进 `m_computeWorkerPool` 的 worker 任务，worker 直接对原始缓冲调 `ChunkSerializer::deserializeChunk(x, z, dataPtr, dataSize)`（指针重载，不再包一层 `vector`）。反序列化结果经 `m_pendingDeserializedChunks` 续延队列回主线程，`update()` 开头 `_processPendingDeserializedChunks` drain 后才 `_applyChunkData` + mesh 调度。同坐标重发用 `m_chunkDeserializeGeneration` 代际号丢弃过期结果。`destroy()` 必须在 `shutdownMeshSystem()` 之前 `waitForCompletion()`，否则在途任务回调会访问已析构的 `this`（且 worker 仍持有网络缓冲视图）。`m_computeWorkerPool` 为 nullptr（测试/启动早期）时走 `_applyDeserializedChunk` 同步 fallback，此时视图由 `onChunkData` 栈上的 `shared_ptr` 保活。**保活是核心**：`shared_ptr` 必须沿调用链传到 worker lambda 捕获，否则 `_processIncomingData`/`poll` 返回后原始缓冲析构 → worker 读悬垂视图 UAF（详见 `src/client/network/README.md` §10）。
- **`processMeshBuildResults()` 只处理最新任务结果**：旧结果会在调度器层丢弃。
- **光照包不要直接当成"立即重建"事件处理**：`onLightSection()` 会先标记 `meshRebuildPending`，如果同一 chunk 的网格任务还在路上，就等当前任务结束后再补提，避免单个 chunk 被光照更新线性打爆。
