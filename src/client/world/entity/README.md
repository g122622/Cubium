...
- `ClientNetwork` - 接收 IR 网络包（`ir::play::AddEntity`/`SetEntityData`/`EntityEvent`/`SetEntityMotion` 等）后调用 spawnEntity/setTargetPosition/setMetadata 等方法（旧 `NetworkClient` 已重命名为 `ClientNetwork`，旧 `EntityMetadataPacket`/`EntityStatusPacket` 类已删除，统一走 IR）
...
7. **元数据同步**：
   - 接收到 `ir::play::SetEntityData`（旧 `EntityMetadataPacket` 已删除）后，调用 `setMetadata()` 设置原始字节（字节流编解码由 `common/network/codec/EntityMetadataSerializer` 负责，从旧 packet/ 迁出）
...
10. **铁傀儡状态不走元数据同步**：
    - 铁傀儡的攻击动画和持花状态通过 `ir::play::EntityEvent`（旧 `EntityStatusPacket` 已删除）触发，**不经过** `ir::play::SetEntityData` / `syncMetadataFromDataManager()`
...
11. **TNT矿车引信计时器不走元数据同步**：
    - TNT矿车的 `fuseTimer` 通过 `mc::network::EntityStatus::EatBlock` (status code 10)（旧 `EntityStatusPacket::Status::EatBlock`）触发，**不经过** `ir::play::SetEntityData` / `syncMetadataFromDataManager()`
    ...
    - 与铁傀儡状态同步模式一致：服务端 `broadcastEntityStatus()` → IR `EntityEvent` 包 → 客户端回调设置字段
...
13. **兔子跳跃动画客户端状态**（参考 MC 1.21.11 `Rabbit.handleEntityEvent(byte 1)`）：
    ...
    - **数据流**：服务端 `RabbitEntity::startJumping()` 广播 `mc::network::EntityStatus::RabbitJump(1)` → `ir::play::EntityEvent`（旧 `EntityStatusPacket` 已删除）→ `onEntityStatus` 回调 → `setRabbitJumpStart()` → `tick()` 推进 → `rabbitJumpCompletion()` → 渲染器 → `RabbitModel`。与狼甩水动画（`mc::network::EntityStatus::ShakeOffWater(8)`）使用相同的"状态包触发 → 客户端镜像状态 → tick 推进 → 渲染器读取"模式。
...
14. **凋灵侧头独立朝向客户端镜像**（参考 MC 1.21.11 `WitherBoss.aiStep()` 中 `j=0..1` 循环）：
    ...
    - **数据流**：服务端 `WitherEntity::aiStep` → `_updateSideHeadRotations`（服务端权威计算）→ `HEAD_TARGET_1/2/3` 通过 `ir::play::SetEntityData`（旧 `EntityMetadataPacket` 已删除）同步 → 客户端 `ClientEntity::syncMetadataFromDataManager` 读取 → `m_witherHeadTargetId[3]` → `ClientEntityManager::tick` 调用 `tickWitherSideHeads` → 客户端独立镜像 MC 计算 → `getInterpolatedWitherSideHeadYaw/Pitch` → `EntityRendererManager` → `WitherModel::setSideHeadRotations` → `WitherModel::setAngles` 应用到 `m_heads[1]`/`m_heads[2]`。
    - **与狼甩水/兔子跳跃模式的差异**：狼甩水/兔子跳跃通过 `ir::play::EntityEvent`（旧 `EntityStatusPacket` 已删除）触发状态镜像 + `tick()` 推进；凋灵侧头通过 `ir::play::SetEntityData` 同步目标 ID + `tickWitherSideHeads` 独立计算朝向。共同点是"服务端权威 → 客户端镜像状态 → tick 推进 → 渲染器读取"。
...
15. **钓鱼浮标网络同步镜像**（参考 MC 1.21.11 `FishingHook.onSyncedDataUpdated()`）：
    ...
    - **数据流**：服务端 `FishingBobberEntity::_syncCaughtEntityId()` / `_catchingFish()` / `tick()` 写入 `DATA_HOOKED_ENTITY_PARAM` / `DATA_BITING_PARAM` → `ir::play::SetEntityData`（旧 `EntityMetadataPacket` 已删除）同步 → 客户端 `syncMetadataFromDataManager` 读取 → `m_fishingHookedEntityId` / `m_fishingBiting` → `FishingBobberRenderer::generateMesh` 消费。
...
16. **Mob 激怒状态镜像**（参考 MC 1.21.11 `Mob.MOB_FLAG_AGGRESSIVE`）：
    ...
    - **数据流**：服务端 `MobEntity::setAggressive(true)` → `DATA_MOB_FLAGS_PARAM` 位 2 置位 → `ir::play::SetEntityData`（旧 `EntityMetadataPacket` 已删除）广播 → 客户端 `syncMetadataFromDataManager` 读取位 2 → `m_isAggressive=true` → `EntityRendererManager::_applyZombieState` 推送到 `ZombieModel::setAggressive` → `ZombieModel::setAngles` 按 `f1 = -PI/(aggressive?1.5:2.25)` 计算手臂前伸基础角度。
