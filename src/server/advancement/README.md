...
21. **PlayerInteractedWithEntityTrigger 触发路径**：不通过 IWorld 回调，而是在 `ServerPlayHandler::_triggerPlayerInteractedWithEntity`（INTERACT/INTERACT_AT 返回 Success 时调用）中按 `AbstractCriterionTrigger<PlayerInteractedWithEntityTriggerInstance>::trigger()` 模板触发，获取玩家手持物品和目标实体进行谓词匹配。（旧 `PacketHandler::handleUseEntity` 已删除，逻辑迁入 `ServerPlayHandler`；`ServerPlayRouter` 为纯转发器不含分支；接线已完成。）
