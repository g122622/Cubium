...
- `setSelectedSlot` 限制范围 0-8（快捷栏）
- `setItem` 限制范围 0-40（快捷栏 + 主背包 + 护甲 + 副手）
- 添加边界检查和日志警告

### 9. 物品栏的权威数据

玩家物品栏只有一份，在玩家实体上（`Player::m_inventory`，经 `Player::inventory()` 访问）：拾取、丢弃、装备、吃东西、存盘与所有游戏逻辑都直接操作它。`InventoryManager` 不持有数据，只按 playerId 定位到这一份并提供同步触发点；定位所需的解析器由服务器注入（交互层拿不到世界上下文）。
