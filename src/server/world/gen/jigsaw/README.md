...
`src/server/world/gen/jigsaw` 目录实现了 Minecraft 1.21 风格的 Jigsaw 拼图结构组装系统，用于动态生成复杂结构（如村庄、掠夺者前哨站、堡垒遗迹、试炼密室）。通过优先级队列算法从模板池中选择并连接拼图块，逐步构建完整结构，并使用 VoxelShape 空间追踪保证结构不重叠、不越界。
