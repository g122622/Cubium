...
- **片段所有权**：所有片段工厂（各家 `createPiece`、`createStrongholdPiece`、`IGameMonumentRoomFitHelper::create` 等）一律返回 `std::unique_ptr`，失败路径返回 `nullptr`；所有权在返回点即确定，调用方用 `pieces.push_back(std::move(...))` 接收。片段的**唯一所有者**是 `StructureStart::m_pieces`（`Structure.hpp`）。例外是 `StrongholdPieces` 中的 `generateAndAddPiece`、`getNextComponentNormal/X/Z`、`StrongholdStartStairs::m_pendingChildren` 与 `m_portalRoom`——这些返回或存储的是**非拥有观察指针**，不得释放，其有效性由同一 `StructureStart` 内的 `m_pieces` 保证
