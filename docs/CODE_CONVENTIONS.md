...
- `try` / `catch` / `throw` 关键字
- `std::exception` 及其派生类
- 可能抛出异常的标准库函数（如 `std::optional::value()` 在空状态下）

**替代方案：**
- 使用 `Result<T>` 类型处理可恢复错误
- 使用 `MC_ASSERT_RELEASE` / `MC_ASSERT_RELEASE_MSG` 处理不可恢复错误

**例外情况：**
- `tests/` 目录中的测试代码可以使用异常（GTest 框架需要）
- 与第三方库交互时，在边界捕获异常并转换为 `Result<T>`
